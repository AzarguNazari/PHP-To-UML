<?php
    ...
    if (isset($_POST['signin'])) {
        ...
        $result = getFullInfo(test_input($_POST['inputEmail']),test_input($_POST['inputPassword'])); // To check if the user's account exist or not

        # if exists then store the information into session
        if(count($result) > 0) {
            
            #   keep all the user's information on session
            ...
            # if there's some error happening during sign in process
                ...
                if(isset($_SESSION["error"])){
                    ...
                    switch ($error) {
                        case "1":
                            ...
                            break;
                        case "2":
                            echo "Please enter a different email";
                            break;
                        case "3":
                            echo "Sorry there's some problem on database";
                            break;
                    }
                }
                echo            '</div>

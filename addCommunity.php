...
if(!(isset($_SESSION['email']) && isset($_SESSION['pass']))){ // if user is not signed in
    $_SESSION['direct'] = "addCommunity.php"; // to bring back the page after signing in
...
if (isset($_POST['communityRegister'])) { 
    ...
    $result = query("select c_id from communities where LOWER(`c_name`)='" . strtolower($c_name) . "'");
    if($result->num_rows > 0){
        $error = "Error! there's already a community with such name, <a href='community.php?id=" . $result->fetch_assoc()['c_id'] . "'>click Here to see</a>";
    }
    else{
        $con = new mysqli("localhost","root","","sp");
        ...
        if ($result) {
            $id = $con->insert_id;
            $con->close();
            $newCon = new mysqli("localhost", "root", "", "chat");
            $result = $newCon->query("create table c$id (id int(11) primary key AUTO_INCREMENT,name varchar(255), msg varchar(255), date timestamp )");
            if($result){
                $newCon->close();
                exit(header("Location: community.php?id=$id"));
            }
            else{
                $error = "Sorry, The chat database is not created";
            }
        } else {
             $error = "sorry the community is not created";
        }
    }
...
<!--                                <label class="text-danger d-none" id="communityNameError"><p><b>Hello world</b></p></label>-->
                            ...
                            <div class="form-check">
                               ...
                               <div class="custom-control custom-radio custom-control-inline">
                                    ...
                                    <div class="custom-control custom-radio custom-control-inline">
                                    <input type="radio" id="no" name="opensource" value="no" class="custom-control-input" checked="checked">
